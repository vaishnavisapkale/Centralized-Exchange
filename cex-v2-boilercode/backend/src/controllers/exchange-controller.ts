...
export async function getUserOrders(req: Request, res: Response): Promise<void> {
  const engineResponse = await sendToEngine("get_user_orders", {
    userId: getUserId(req),
  });
...
export async function getRecentTrades(req: Request, res: Response): Promise<void> {
  ...
  const { symbol } = parsedParams.data;
  const engineResponse = await sendToEngine("get_recent_trades", { symbol });
...
export async function depositBalance(
  ...
  const engineResponse = await sendToEngine("deposit_balance", {
    userId: getUserId(req),
    asset,
    amount,
  });
